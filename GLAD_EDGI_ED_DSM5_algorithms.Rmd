...
edgi.mhd.raw.id <- edgi.mhd.raw %>%
    ...
    select(
     ...
     mhd.atypical_anoreixa_signup=mhd.atypical_anorexia_nervosa
...
For participants who have not self-reported their BMI, we calculate it using their height and weight: The BMI is defined as the body mass divided by the square of the body height, and is universally expressed in units of kg/m², resulting from mass in kilograms and height in metres.
...
NB: The GLAD signup does not ask for current BMI (but does ask about height + weight - so need to derive it)
```{r }
...
##Gerome check - age of onset - EDGI
```{r}
#BED
BED_EDGI <- dat.ed.EDGI %>%
...
BED_EDGI %>%
  filter(be.began_roughly_regular_episodes.txt_numeric < 16) %>%
  count()
...
AN_EDGI %>%
  filter(an.how_old_were_you_then.txt_numeric < 16) %>%
  count()

```

##Gerome check 2 - age of onset under 26 and trumping of diagnosis by age of onset - EDGI

```{r}

dat.ed.EDGI$AN_trumped <-
  with(dat.ed.EDGI,
  if_else(
    (
      (
      ...
      an.how_old_were_you_then.txt_numeric < be.began_roughly_regular_episodes.txt_numeric
    ) |
    ...
    (
       anorexia_group_numeric == 1 &
        (
          (BED_group_numeric == 0 |
           is.na(BED_group_numeric)) &
            (bulimia_group_numeric == 0 |
           is.na(bulimia_group_numeric))
           )
            )
    ),
      true = 1,
  ...
  if_else(
    ...
    (
      ...
      (
        bulimia_group_numeric == 1 &
        (
          (anorexia_group_numeric == 0 |
           is.na(anorexia_group_numeric)) &
            (bulimia_group_numeric == 0 |
           is.na(bulimia_group_numeric))
           )
            )
...
```    
#Table of trumped diagnoses
```{r}

table(dat.ed.EDGI$BED_trumped,
      dat.ed.EDGI$BN_trumped)

table(dat.ed.EDGI$AN_trumped,
      dat.ed.EDGI$BED_trumped)
...
#BED
BED_EDGI <- dat.ed.EDGI %>%
  filter(BED_trumped == 1) 

nrow(BED_EDGI)
  

BED_EDGI %>%
  filter(be.began_roughly_regular_episodes.txt_numeric < 26) %>%
  count()
...
AN_EDGI %>%
  filter(an.how_old_were_you_then.txt_numeric < 26) %>%
  count()
...
##Gerome check - age of onset - GLAD
```{r}
#BED
BED_GLAD <- dat.ed.GLAD %>%
  filter(BED_group_numeric == 1) 
...
AN_GLAD %>%
  filter(an.how_old_were_you_numeric < 16) %>%
  count()

```

##PGC check - Lowest BMI in self-reported AN and AAN
...
#Percentage with BMI over 20
mhd.self.report.GLAD %>%
  filter(LOWEST_BMI > 20) %>%
  nrow()

(11/189)*100

```
#2. ATYPICAL ANOREXIA mhd self report = Have you ever been diagnosed with one or more of the following mental health problems or neurodevelopmental disorders by a professional, even if you don't have it currently? By professional we mean: any doctor, nurse or person with specialist training . Please include disorders even if you did not need treatment for them or if you did not agree with the diagnosis. Select ALL that apply:

```{r}

##Identify group

#Filter for those who self-report AAN
mhd.self.report.AAN.GLAD <- dat.ed.GLAD %>%
  filter(mhd.atypical_anorexia_signup_numeric == 1)

nrow(mhd.self.report.AAN.GLAD)

#BMI variable - only available in the signup (not asked in AAN module)
mhd.self.report.AAN.GLAD$LOWEST_BMI <- NA_real_
  
mhd.self.report.AAN.GLAD$LOWEST_BMI <- 
  if_else(
    condition = is.na(mhd.self.report.AAN.GLAD$LOWEST_BMI),
    true = mhd.self.report.AAN.GLAD$low_dem_bmi,
    false = NA_real_,
    missing = NA_real_
    )

#Descriptives
mhd.self.report.AAN.GLAD %>%
  freq(LOWEST_BMI)
...
#Plot 
qplot(mhd.self.report.AAN.GLAD$LOWEST_BMI)
...
#2. Atypical AN self report = Which of the following eating disorders did you/do you currently have? If you have not received a diagnosis and are not sure which of the following may apply to you please select Don't know: [this is asked to people who indicate "yes" to "Have you ever suspected that you may have an eating disorder, whether or not you were ever diagnosed?"]
...
#Filter for those who self-report AN diagnosis
aan.self.report.GLAD <- dat.ed.GLAD %>%
  filter(an.1.atypical_anorexia_nervosa_numeric == 1)

nrow(aan.self.report.GLAD)

#BMI variable - if not reported in signup, use value from ED100K (if available)
aan.self.report.GLAD$LOWEST_BMI  <- NA

aan.self.report.GLAD$LOWEST_BMI <- 
  if_else(
    condition = is.na(aan.self.report.GLAD$LOWEST_BMI),
    true = aan.self.report.GLAD$low_dem_bmi,
    false = NA_real_,
    missing = NA_real_
    )

#Descriptives
aan.self.report.GLAD %>%
  freq(LOWEST_BMI)
...
#Plot 
qplot(aan.self.report.GLAD$LOWEST_BMI)
