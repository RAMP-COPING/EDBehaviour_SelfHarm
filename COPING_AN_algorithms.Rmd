...
### Read in ED data: NBR DEM (need people's height)
```{r}
dat.dem.nbr <- readRDS(file = "../data_raw/nbr/demographics_coping_nbr.rds")

# Inspect dimensions
dim(dat.dem.nbr)
# Inspect colnames
colnames(dat.dem.nbr)

nrow(dat.dem.nbr)

```
### Select variables: NBR DEM
```{r glad select variables}

exclude_cols_dem <- c("ID",
                  "cohort_name")

dat.dem.nbr.id <- dat.dem.nbr %>% #new dataset with ID
  drop_na(subjectid) %>% # Drop NAs
  distinct(subjectid, .keep_all = TRUE) %>% 
  add_column(cohort_name = "coping_nbr", .before = "demographics.what_is_your_current_height") %>% #create new column 
  select(
         ID = subjectid,# ID
         cohort_name, # Sample
         dem.what_is_your_current_height = demographics.what_is_your_current_height,
         dem.what_is_your_current_height.1 = demographics.what_is_your_current_height.1,
         dem.what_is_your_current_height.2  =demographics.what_is_your_current_height.2                   
         ) %>%
  add_numeric(., exclude = exclude_cols_dem) %>%
  mutate_if(is.numeric, ~na_if(., -88)) %>% # Recode missing values to NAs in numeric variables
  mutate_if(is.numeric, ~na_if(., -99)) %>%
...
dim(dat.dem.nbr.id)
# Inspect colnames
colnames(dat.dem.nbr.id)
#Differences
dim(dat.dem.nbr)[1]-dim(dat.dem.nbr.id)[1]
