...
## Final cleaned data
```{r Read in data}
dat <- readRDS(file = "../data_cleaned/final_dat_merged_2022-03-24.rds")
...
```{r n of people excluded due to prepan experience}
dat <- dat %>%
  mutate(excluded_prepan =
           case_when(is.na(lifetime_binge_eating_baseline_numeric) &
                       is.na(lifetime_low_weight_baseline_numeric) &
                       is.na(PREPANDEMIC_passive_suicidal_ideation) &
                       is.na(PREPANDEMIC_selfharm_ideation) &
                       is.na(PREPANDEMIC_selfharm) ~ "Missing for all",
                      ...
                      PREPANDEMIC_selfharm == 0 ~ "No pre-pandemic experience for at least one",
                     
                      lifetime_binge_eating_baseline_numeric == 1 |
                      lifetime_low_weight_baseline_numeric == 1 |
                      PREPANDEMIC_passive_suicidal_ideation== 1 |
                      PREPANDEMIC_selfharm_ideation== 1 |
                      PREPANDEMIC_selfharm == 1 ~ "Excluded because of pre-pandemic experience"
           ))
                     
dat %>%
  freq(excluded_prepan)

# Full sample n
dat %>%
  filter(excluded_prepan == "No pre-pandemic experience for at least one" &
...
freq(dat$NEW_binge_eating_pandemic)
```
