...
# Read in data
```{r Read in data}
dat <- readRDS(file = "../data_cleaned/final_dat2022-01-25.rds")
dim(dat)
   colnames(dat)
```
...
## Binge eating - extended model 25+ years only
```{r create dataset of 25+ only}
dat_over_25 <- dat %>%
  filter(age_category_COVID_baseline != "16-25")
```

```{r Binge eating extended model over 25 only}
model6a <- glm(NEW_binge_eating_pandemic ~  
                 age_category_COVID_baseline +
                 gender_cleaned +
                 race_binary +
                 highest_education +
...
binge_eating_main <- tidy(model6a,
     conf.int=TRUE,
     exponentiate=TRUE)
...
# Pseudo R2
DescTools::PseudoR2(model6a, which = "McFadden")
DescTools::PseudoR2(model6a, which = "McFaddenAdj")
DescTools::PseudoR2(model6a, which = "Nagelkerke")

```

Plot
```{r Binge eating extended model plot}
dat_plot_over_25 <- dat_over_25 %>%
  select("Age" = "age_category_COVID_baseline",
         "Gender" = "gender_cleaned",
         "Race" = "race_binary",
         "Pandemic_binge_eating" = "NEW_binge_eating_pandemic",
...
BE_extended_dat_plot_over_25 <- dat_plot_over_25 %>%
  finalfit::or_plot(dependent,
                    explanatory, 
          breaks = c(0.5, 1, 5, 10, 20, 30),
          table_text_size = 3.0,
          title_text_size = 15,
          dependent_label = "New experience of binge eating during pandemic (25+ years only)",
          remove_ref = FALSE) 
```

```{r Save BE extended level plot over 25}
ggsave(
  paste0("BE_extended_dat_plot_over_25",
                    date,
                    ".png"),
  plot = BE_extended_dat_plot_over_25,
  device = "png", 
  path = "/Users/helenadavies/werk/EDBEHSELFHARM2/gitEDBEHSELFHARM/plots/extended", 
  scale = 1,
  width = 25,
...
# Low weight - extended model (25+ years)
Age, gender, race, education, psych disorder, key worker/employment status, vulnerable group member, pandemic worry, positive COVID test, loss of loved one, lonelieness, change in main eco activity, change in living situation
```{r Low weight extended model 25+ years}
model7a <- glm(NEW_low_weight_pandemic ~ age_category_COVID_baseline +
                 gender_cleaned +
                 race_binary +
                 highest_education +
                 psych_disorder_factor +
...
# Pseudo R2
DescTools::PseudoR2(model7a, which = "McFadden")
DescTools::PseudoR2(model7a, which = "McFaddenAdj")
DescTools::PseudoR2(model7a, which = "Nagelkerke")
```

Plot
```{r Low weight extended model plot 25+ years}
dat_plot_over_25 <- dat_over_25 %>%
  select("Age" = "age_category_COVID_baseline",
         "Gender" = "gender_cleaned",
         "Race" = "race_binary",
...
LW_extended_dat_plot_over_25 <- dat_plot_over_25 %>%
  finalfit::or_plot(dependent,
                    explanatory, 
          breaks = c(0.5, 1, 5, 10, 20, 30),
          table_text_size = 3.0,
          title_text_size = 15,
          dependent_label = "New experience of low weight during pandemic (25+ years)",
           remove_ref = FALSE) 
```

```{r Save LW extended level plot}
ggsave(
  paste0("LW_extended_dat_plot_over_25",
                    date,
                    ".png"),
  plot = LW_extended_dat_plot_over_25,
  device = "png", 
  path = "/Users/helenadavies/werk/EDBEHSELFHARM2/gitEDBEHSELFHARM/plots/extended", 
  scale = 1,
  width = 25,
...
# Harmful ideation - extended model (25+ years)
Age, gender, race, education, psych disorder, key worker/employment status, vulnerable group member, pandemic worry, positive COVID test, loss of loved one, lonelieness, change in main eco activity, change in living situation
```{r Harmful ideation extended model 25+ years}
model8a <- glm(NEW_harmful_ideation_pandemic ~ age_category_COVID_baseline + 
                 gender_cleaned + 
                 race_binary + 
                 highest_education + 
                 psych_disorder_factor + 
...
harmful_ideation_main <- tidy(model8a,
     conf.int=TRUE,
     exponentiate=TRUE)
...
# Psuedo R2
DescTools::PseudoR2(model8a, which = "McFadden")
DescTools::PseudoR2(model8a, which = "McFaddenAdj")
DescTools::PseudoR2(model8a, which = "Nagelkerke")
```


Plot
```{r Harmful ideation extended model plot 25+ years}
dat_plot_over_25 <- dat_over_25 %>%
  select("Age" = "age_category_COVID_baseline",
         "Gender" = "gender_cleaned",
         "Race" = "race_binary",
...
HI_extended_dat_plot_over_25 <- dat_plot_over_25 %>%
  finalfit::or_plot(dependent,
                    explanatory, 
          breaks = c(0.5, 1, 5, 10, 20, 30),
          table_text_size = 3.0,
          title_text_size = 13,
          dependent_label = "Harmful ideation during pandemic (25+ years only)",
           remove_ref = FALSE) 
```

```{r Save HI extended level plot 25+ years}
ggsave(
  paste0("HI_extended_dat_plot_over_25",
                    date,
                    ".png"),
  plot = HI_extended_dat_plot_over_25,
  device = "png", 
  path = "/Users/helenadavies/werk/EDBEHSELFHARM2/gitEDBEHSELFHARM/plots/extended", 
  scale = 1,
  width = 25,
...
# Self-harm - extended model (25+ years)
Age, gender, race, education, psych disorder, key worker/employment status, vulnerable group member, pandemic worry, positive COVID test, loss of loved one, lonelieness, change in main eco activity, change in living situation
```{r Self-harm extended model 25+ years}
model10a <- glm(NEW_selfharm_pandemic ~ age_category_COVID_baseline + 
                 gender_cleaned + 
                 race_binary + 
              #   highest_education + 
                 psych_disorder_factor + 
...
# Psuedo R2
DescTools::PseudoR2(model10a, which = "McFadden")
DescTools::PseudoR2(model10a, which = "McFaddenAdj")
DescTools::PseudoR2(model10a, which = "Nagelkerke")
```

Plot 
```{r Self-harm extended model plot 25+ years}
dat_plot_over_25 <- dat_over_25 %>%
  select("Age" = "age_category_COVID_baseline",
         "Gender" = "gender_cleaned",
         "Race" = "race_binary",
...
SH_extended_dat_plot_over_25 <- dat_plot_over_25 %>%
  finalfit::or_plot(dependent, explanatory, 
          breaks = c(0.5, 1, 5, 10, 20, 30),
          table_text_size = 3.0,
          title_text_size = 13,
          dependent_label = "Self-harm during pandemic (25+ years only)",
          remove_ref = FALSE)
```


```{r Save SH extended level plot 25+ years}
ggsave(
  paste0("SH_extended_dat_plot_over_25",
                    date,
                    ".png"),
  plot = SH_extended_dat_plot_over_25,
  device = "png", 
  path = "/Users/helenadavies/werk/EDBEHSELFHARM2/gitEDBEHSELFHARM/plots/extended", 
  scale = 1,
  width = 25,
...
# Sensitivity analysis +++take out education in these models as well?
