...
dat.be.coping.glad <- readRDS(file = paste0(filepath_raw_latest_freeze, "coping_glad/be_coping_glad.rds"))
# Inspect dimensions
...
dat.be.nbr <- readRDS(file = paste0(filepath_raw_latest_freeze, "coping_nbr/be_coping_nbr.rds"))
...
dat.an.scr.glad.coping <- readRDS(file = paste0(filepath_raw_latest_freeze, "coping_glad/an_scr_coping_glad.rds"))
...
```{r Read in data GLAD AN screener}
dat.an.glad.coping <- readRDS(file = paste0(filepath_raw_latest_freeze, "coping_glad/an_coping_glad.rds"))
...
dat.an.scr.nbr <- readRDS(file = paste0(filepath_raw_latest_freeze, "coping_nbr/an_scr_coping_nbr.rds"))
...
```{r Read in data NBR AN SCREENER}
dat.an.nbr <- readRDS(file = paste0(filepath_raw_latest_freeze, "coping_nbr/an_coping_nbr.rds"))
...
dat.mhd.glad.coping <- readRDS(file = paste0(filepath_raw_latest_freeze, "coping_glad/mhd_coping_glad.rds"))
...
dat.mhd.nbr.coping <- readRDS(file = paste0(filepath_raw_latest_freeze, "coping_nbr/mhd_coping_nbr.rds"))
...
age_dat_nbr <- readRDS(file = paste0(filepath_cleaned_latest_freeze, "coping_nbr/demographics/age_coping_nbr_clean_INTERNAL_ONLY.rds"))
dim(age_dat_nbr)
...
age_dat_glad_coping <- readRDS(file =  paste0(filepath_cleaned_latest_freeze, "coping_glad/demographics/age_coping_glad_clean_INTERNAL_ONLY.rds"))
...
## GLAD COPING
```{r Merge GLAD data}
dat.ed.GLAD.COPING <- list(dat.be.coping.glad.id,
...
## How many people have binge eating that might have started during the pandemic?
...
dat.ed.COPING <- dat.ed.COPING %>%
  mutate(age_start_pandemic =
           case_when(
             !is.na(age_start_pandemic_cop) ~ age_start_pandemic_cop, # if DOB available in COPING use COPING
             !is.na(age_start_pandemic_glad) ~ age_start_pandemic_glad  # if DOB not available in COPING, use GLAD
             )
         )
```

### Binge eating
NB: People will get routed to the ED100K if they 1) endorse the screener, 2) enodrse an AN diagnosis, 3) have a BMI lower than 18.5 as calculcated from the demographics. Therefore, we do not need to include the question about low weight "an.weigh_weighed_people_thought" as some people may not have endorsed this but will have still answered the questions about low weight.
  ...
  mutate(pandemic_BE_vs_start_pandemic_age =
           case_when(
            ...
            # be.ate_regard_eating_binges == "Yes" & 
             age_start_pandemic < be.roughly_binge_eating_regular.txt ~ "Pandemic binge eating", # they've turned that age since the pandemic started (i.e., started binge eating during the pandemic)
             
            #  be.ate_regard_eating_binges == "Yes" & 
             age_start_pandemic > be.roughly_binge_eating_regular.txt ~ "Pre-pandemic binge eating", # they started binge eating at an age younger than their age at the beginning of the pandemic (i.e., started binge eating BEFORE the pandemic)
...
dat.ed.COPING %>%
  ...
  select(mhd_EndDate,
         ...
         be.roughly_binge_eating_regular.txt)
```
17/01/22: There are 116 people who were the same age at the start of the pandemic as their age when they started binge eating so we can not work out whether they started before or during the pandemic. Will have to drop. 
...
dat.ed.COPING <- dat.ed.COPING %>%
  mutate(any_pandemic_binge_eating_vs_start_pandemic_age =
           case_when(   # People who indicated an age at symptom YOUNGER than their age at the start of the pandemic, or ANY diagnosis (given diagnostic delays, very likely symptoms started before the pandemic)
                       ...
                       mhd.atypical_bingeeating_disorder == "Atypical binge-eating disorder" |
                        ...
                        mhd.atypical_bulimia_nervosa == "Atypical bulimia nervosa"  ~ "Pre-pandemic binge eating",
                     
                         # People who indicated an age at symptom OLDER than their age at the start of the pandemic
                       pandemic_BE_vs_start_pandemic_age == "Pandemic binge eating"  ~ "Pandemic binge eating",
                    
                        # The rest may have indicated that they have never binged, or no age info, or reported the same age
...
```
17/01/22: There are 12 people who we have been able to identify as having pandemic binge eating (i.e., binge eating that started during the pandemic).
...
dat.ed.COPING <- dat.ed.COPING %>%
  mutate(lifetime_binge_eating_baseline_numeric =
           ...
           # People who, for binge eating, indicated pandemic start and therefore, before the pandemic had NOT experienced binge eating  *will need to code these people as baseline binge eating*
        ...
        # Missing age data
          any_pandemic_binge_eating_vs_start_pandemic_age == "Missing data" ~ 2
        )
      )
...
```
17/01/22: 33 people had answered "No" to ever binge eating, but then not answered the MHD questionnaire, or said "Yes" to suspected eating disorder but then not indicated which ED they suspected they have had, or put "Not sure". For these people, have decided to count them as "Not lifetime binge eating" as it is the symptom we are most interested in rather than if they have had any other eating disorder.

Now, there are 3,133 people who we can not classify. Either 'Yes' or missing for the BE question. If "Yes" for BE questions, we've been unable to work out when they started, e.g., included age at start of pandemic but not age at which they started binge eating. And all are either "no" or 'Not sure' for suspected ED, or "Yes" but then "Not bulimia nervosa", "Not Atypical bulimia nervosa", "Not Binge-eating disorder", "Not Atypical binge-eating disorder".
...
dat.ed.COPING <- dat.ed.COPING %>%
  mutate(pandemic_LW_vs_start_pandemic_age =
         ...
         #  an.weigh_weighed_people_thought == "Yes"  & # Had lifetime low weight
             age_start_pandemic < an.1.how_old_were_you_then.txt ~ "Pandemic low weight", # they've turned that age since the pandemic started (i.e., low weight started during the pandemic)
             
          #    an.weigh_weighed_people_thought == "Yes"  & # Had lifetime low weight
             age_start_pandemic > an.1.how_old_were_you_then.txt ~ "Pre-pandemic low weight", #  started low weight at an age younger than their age at the beginning of the pandemic
...
dat.ed.COPING %>%
  ...
  select(mhd_EndDate,
         ...
         an.1.how_old_were_you_then.txt)
```
17/01/22: There are 234 people who put the same age at start of low weight as their age at the start of the pandemic so we can not work out whether they started before or during the pandemic. Will have to drop. 
...
dat.ed.COPING <- dat.ed.COPING %>%
  mutate(any_pandemic_low_weight_vs_start_pandemic_age =
           case_when(   # People who indicated an age at symptom start YOUNGER than their age at the start of the pandemic
                       pandemic_LW_vs_start_pandemic_age == "Pre-pandemic low weight"  ~ "Pre-pandemic low weight",
                     
                         # People who indicated an age at symptom start OLDER than their age at the start of the pandemic
                       pandemic_LW_vs_start_pandemic_age == "Pandemic low weight"   ~ "Pandemic low weight" 
                     )
         )
dat.ed.COPING %>%
...
dat.ed.COPING <- dat.ed.COPING %>%
  mutate(lifetime_low_weight_baseline_numeric =
           case_when(
            # (See code above)
            # People who, for low weight, indicated prepandemic (taking into account age at pandemic start etc)
              any_pandemic_low_weight_vs_start_pandemic_age ==  "Pre-pandemic low weight"  ~ 1,
              
           # People who, for low weight, indicated pandemic start and therefore, before the pandemic, had NOT experienced low weight *will need to code these people as baseline low weight*
       ...
       an.weigh_weighed_people_thought == "No" ~ 0,
        ...
        # Missing age data
          any_pandemic_low_weight_vs_start_pandemic_age == "Missing data" ~ 2
...
dat.ed.COPING %>%
  ...
  select(
    sample,
    any_pandemic_low_weight_vs_start_pandemic_age,
           an.weigh_weighed_people_thought,
           an.1.how_old_were_you_then.txt,
           age_start_pandemic,
           mhd.suspected_eating_disorder_diagnosed,
...
```
17/01/22: 31 people have answered "No" to ever being of low weight, but then not answered the MHD questionnaire, or said "Yes" to suspecting an eating disorder but then not indicated which ED they suspected they have had, or put "Not sure". For these people, have decided to count them as "Not lifetime low weight" as it is the symptom we are most interested in rather than if they have had any other eating disorder.

Now, there are 1,076 people who we can not classify. Either 'Yes' or missing for the LW question. And 'Not sure', 'Yes' (but not indicated which EDs), 'No' (but 'Yes' or NA for LW) for the MHD.
...
glad_ed100k_selfreport_diagnoses <- readRDS(file = paste0(filepath_cleaned_latest_freeze, "glad/eating/selfreported_ed100k_diagnoses_glad_clean.rds"))
...
glad_mhd <- readRDS(file =   paste0(filepath_raw_latest_freeze, "glad/mhd_glad.rds"))
dim(glad_mhd)
colnames(glad_mhd)
...
AN screener
```{r Read in data glad an screener}
glad_AN_scr <- readRDS(file =   paste0(filepath_raw_latest_freeze, "glad_ed/an_scr_glad.rds"))
dim(glad_AN_scr)
...
glad_AN_age <- readRDS(file =   paste0(filepath_raw_latest_freeze, "glad_ed/an_glad_ed.rds"))
dim(glad_AN_age)
...
glad_BE <- readRDS(file =   paste0(filepath_raw_latest_freeze, "glad_ed/be_glad_ed.rds"))
dim(glad_BE)
colnames(glad_BE)
...
glad_dat <- dplyr::left_join(glad1,
...
glad_dat <- glad_dat %>%
  mutate(lifetime_binge_eating_baseline_GLAD_numeric =
           case_when(
            ...
            # DIAGNOSIS LEVEL - NO AGE DATA (NB: this is fine because anyone with a diagnosis during the pandemic is likely to have had pre-pandemic symptoms given diagnostic delays)
       ...
       # Capture people who answered BEFORE the pandemic
          ...
          # Self-reported binge eating ED100K 
             be.short_period_ate_regard == "Yes" ~ 1, # Pre-pandemic binge eating (all people with a binge-type ED are captured above)
      ...
      # Capture people who answered DURING the pandemic (so need to check ages)    
           ...
           #  Pre-pandemic binge eating
            (endDate_ED100K_BE >= as.Date("2020-03-23") &  # answered during pandemic
             be.short_period_ate_regard == "Yes" & # endorsed binge eating
            age_start_pandemic >= be.began_roughly_regular_episodes.txt ~ 1), # started binge eating before the pandemic, need to drop
           
           # Newly experienced binge eating DURING pandemic
             (endDate_ED100K_BE >= as.Date("2020-03-23") & # answered during pandemic
             be.short_period_ate_regard == "Yes" & # endorsed binge eating
...
glad_dat <- glad_dat %>%
  mutate(lifetime_low_weight_baseline_GLAD_numeric =
    ...
    # Capture people who answered BEFORE the pandemic
          ...
          an.1.people_thought_weigh_weighed == "Yes" ~ 1, # Endorsed low weight = prepandemic low weight
           ...
           endDate_ED100K_AN < as.Date("2020-03-23")) & 
            ...
            an.1.anorexia_nervosa == "Not Anorexia nervosa") ~ 0,  # No pre-pandemic experience of low weight
          
          
    # Capture people who answered DURING the pandemic (so need to check ages)    
            
           # Pre-pandemic low weight
            (endDate_ED100K_AN >= as.Date("2020-03-23") &  # answered during pandemic
             an.1.people_thought_weigh_weighed == "Yes" & # endorsed low weight
           age_start_pandemic >= an.how_old_were_you_then ~ 1), # low weight occurred before pandemic = pre-pandemic low weight (need to drop)
          
          # Newly experienced low weight DURING pandemic 
             (endDate_ED100K_AN >= as.Date("2020-03-23") &  # answered during pandemic
             an.1.people_thought_weigh_weighed == "Yes" & # endorsed low weight
             age_start_pandemic <  an.how_old_were_you_then ~ 9), # low weight occurred after start of pandemic = pandemic low weight 
           
         # No low weight or anorexia at baseline (answered during pandemic; may go on to experience it during)
             endDate_ED100K_AN >= as.Date("2020-03-23") &  # answered during pandemic
            an.1.people_thought_weigh_weighed == "No" & # no low weight
            mhd.an == 0 & # No self-report diagnosis of anorexia
       ...
       # Just in case, capture those left over. 
         ...
         # Optional ED100K: For those who filled out the optional ED100K, they are likely to have done so AFTER the main GLAD sign-up, so this should come first as this captures more recent info
          ...
          # Added 17/01/22: Capture those with symptom-only data
           ...
           endDate_ED100K_AN < as.Date("2020-03-23")) &  
            # No self-reported low weight ED100K 
...
ramp.mhd.raw <- readRDS(file = paste0(filepath_cleaned_latest_freeze, "ramp/clinical/mhd_ramp_clean.rds"))
dim(ramp.mhd.raw)
...
ramp.edeq.scr.baseline <- readRDS(file =  paste0(filepath_raw_latest_freeze, "ramp/ed_scid5_ramp.rds"))
...
```{r Read in data RAMP EDEQ baseline}
ramp.edeq.baseline <- readRDS(file =  paste0(filepath_saved_data, "BASELINE_ramp2022-09-27.rds"))
...
ramp_dat <- ramp_dat %>%
  mutate(lifetime_binge_eating_baseline_numeric =
           case_when(
              ...
              mhd.bed_cop_numeric == 1 ~ 1, # Likely BE before the pandemic although this will capture people with pandemic-starting binge eating but no age data available to check this)
            ...
            (edeq.regard_lost_control_eaten.1_.Wavebaseline_numeric == 0 | # Indicated they had binged in the past month BUT not in the 28 days before the pandemic (so assuming this has occurred during the pandemic - will be captured by outcome code)
            is.na(edeq.regard_lost_control_eaten.1_.Wavebaseline_numeric)) # ...did not get shown question about binge eating before the pandemic because they did not pass the screener
               ~  0,
...
ramp_dat %>%
  freq(lifetime_low_weight_baseline_numeric)
```
17/01/22: There are 8591 people missing (i.e., have put PNTA/Don't know/Seen but not answered for the MHD anorexia question at RAMP baseline)
...
edgi.mhd.raw <- readRDS(file = paste0(filepath_raw_latest_freeze, "edgi/mhd_edgi.rds"))
dim(edgi.mhd.raw)
...
edgi_an <- readRDS(file = paste0(filepath_raw_latest_freeze, "edgi/an_edgi.rds"))
dim(edgi_an)
colnames(edgi_an)
```
...
be.edgi.raw <- readRDS(file = paste0(filepath_raw_latest_freeze, "edgi/be_edgi.rds"))
dim(be.edgi.raw)
...
age_dat_edgi_coping <- readRDS(file =  paste0(filepath_cleaned_latest_freeze, "coping_edgi/demographics/age_coping_edgi_clean_INTERNAL_ONLY.rds"))
...
edgi_dat <- edgi_dat %>%
  mutate(lifetime_binge_eating_baseline_numeric =
     ...
     # Capture those who answered EDGI BEFORE the pandemic
           ...
           # ANY Binge-type eating disorder (NB: Doesn't matter age at diagnosis because symptoms likely to have started before the pandemic given diagnostic delays)
             (mhd.bingeeating_disorder == "Binge-eating disorder" |
             mhd.bulimia_nervosa == "Bulimia nervosa" |
             mhd.atypical_bingeeating_disorder == "Atypical binge-eating disorder" |
           ...
           endDate_BE < as.Date("2020-03-23") & # Answered BE questionnaire before the pandemic
            ...
            endDate_BE < as.Date("2020-03-23") & # Answered BE questionnaire before the pandemic
             ...
             (be.short_period_ate_regard == "No" & # No pre-pandemic binge eating
                ...
                # Suspected eating disorder but not a binge-type one
    ...
    # Capture those who answered EDGI DURING the pandemic 
           ...
           # Newly experienced binge eating DURING the pandemic 
            endDate_BE >= as.Date("2020-03-23") & # Answered BE questionnaire DURING the pandemic
            be.short_period_ate_regard == "Yes" & # endorsed binge eating
...
```
17/01/22: There are 1,544 people in EDGI who we can not classify. 185 have not answered BE questions. Other have put "Yes" and answered it during the pandemic BUT are missing 'age at start of pandemic' data (only have this for people who answered COPING; only using data from COPING participants anyway) or BE symptom start data.
...
edgi_dat <- edgi_dat %>%
  mutate(lifetime_low_weight_baseline_numeric =
        ...
        # Capture those who answered EDGI BEFORE the pandemic
           ...
           # ANY Anorexia diagnosis (NB: Doesn't matter age at diagnosis because symptoms likely to have started before the pandemic given diagnostic delays)
      ...
      # Capture those who answered EDGI DURING the pandemic: need to check ages
          ...
          # Pre-pandemic low weight
             endDate_AN >= as.Date("2020-03-23") & # Answered AN questionnaire during the pandemic
              mhd.people_thought_weigh_weighed == "Yes" & # endorsed low weight
             age_start_pandemic >= an.how_old_were_you_then.txt_numeric ~ 1,  # started before the pandemic (need to drop)
          
         # Newly experienced low weight DURING the pandemic
              endDate_AN >= as.Date("2020-03-23") & # Answered AN questionnaire during the pandemic
             mhd.people_thought_weigh_weighed == "Yes" & # endorsed low eight
           ...
           endDate_AN >= as.Date("2020-03-23") & # Answered AN questionnaire during the pandemic
           mhd.people_thought_weigh_weighed == "No" & # No low weight
           mhd.anorexia_nervosa == "Not Anorexia nervosa" ~ 0,  # i.e., has suspected ED but not anorexia nervosa
...
```
17/01/22: There are 1,016 people in EDGI who we can not classify. 214 have not answered LW questions. Other have put "Yes" and answered it during the pandemic BUT are missing 'age at start of pandemic' data (only have this for people who answered COPING; only using data from COPING participants anyway) or LW symptom start data. 
...
edgi_dat <- edgi_dat %>%
  mutate(lifetime_binge_eating_baseline_numeric =
           case_when(
             pandemic_binge_eating_derived == 1 ~ 0,
             TRUE ~ lifetime_binge_eating_baseline_numeric
           )
  )
...
edgi_dat <- edgi_dat %>%
  mutate(lifetime_low_weight_baseline_numeric =
           case_when(
             pandemic_low_weight_derived == 1  ~ 0,
             TRUE ~ lifetime_low_weight_baseline_numeric
           )
  ) 
```
...
# MERGE ALL DATA
```{r Merge all data}
baseline_ED_groups <- ramp_dat %>% # RAMP data
  bind_rows(
      edgi_dat,  # Only EDGI sign-up (no ED100K in EDGI COPING)
      glad_merged, # Includes sign-up and GLAD COPING data (ED100K in both)
      nbr_dat) # NBR COPING
...
saveRDS(object = baseline_ED_groups,
        file = paste0(filepath_saved_data, "ED_BASELINE_groups", date, ".rds"))
```
