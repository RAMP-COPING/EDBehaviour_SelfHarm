...
##Look at numbers for sample sizes of stratified plots - 
```{r}
...
```
##Check no. of participants who are NOT missing for EVERY life not worth living 2 weeks variable at EVERY wave
```{r}

dat.life.worth.living.2.weeks <- dat.taf %>%
  filter(
    !is.na(taf.pandemic_felt_.Wavebaseline) |
    !is.na(taf.felt_weeks_past_.Wavebaseline) |
    !is.na(`taf.felt_weeks_past_.Wave 0`) |
    !is.na(`taf.felt_weeks_past_.Wave 1`) |
    !is.na(`taf.felt_weeks_past_.Wave 2`) |
      !is.na(`taf.felt_weeks_past_.Wave 3`) |
      !is.na(`taf.felt_weeks_past_.Wave 4`) |
      ...
      !is.na(`taf.felt_weeks_past_.Wave 9`)
  )

nrow(dat.life.worth.living.2.weeks)   
...
##Check no. of participants who are NOT missing for EVERY contemplated harm 2 weeks variable at EVERY wave
```{r}

dat.contemplated.harm.2.weeks <- dat.taf %>%
  filter(
    !is.na(taf.pandemic_felt.1_.Wavebaseline) |
    !is.na(taf.felt_weeks_past.1_.Wavebaseline) |
    !is.na(`taf.felt_weeks_past.1_.Wave 0`) |
    !is.na(`taf.felt_weeks_past.1_.Wave 1`) |
    !is.na(`taf.felt_weeks_past.1_.Wave 2`) |
      !is.na(`taf.felt_weeks_past.1_.Wave 3`) |
      !is.na(`taf.felt_weeks_past.1_.Wave 4`) |
      ...
      !is.na(`taf.felt_weeks_past.1_.Wave 9`)
  )

nrow(dat.contemplated.harm.2.weeks)  
...
levels(dat.no.nas$wave_factor)[11] <- "January 2021"
...
##Check number of psych disorder cases in people wiht non-missing data
dat.life.worth.living.2.weeks.psych <- dat.life.worth.living.2.weeks %>%
  filter(control_numeric == 0)

nrow(dat.life.worth.living.2.weeks.psych)
```
...
##Check number of psych disorder cases in people wiht non-missing data
dat.life.worth.living.2.weeks.psych1 <- dat.life.worth.living.2.weeks %>%
  filter(control_numeric == 1)

nrow(dat.life.worth.living.2.weeks.psych1)
...
```
##Check how many participants answered each wave RAMP: taf.felt_weeks_past.1
```{r}

dat.taf.retroprepan <- dat.taf %>%
  filter(wave == "retro_prepan" &
           cohort_name == "ramp" &
           !is.na(taf.felt_weeks_past.1)
         )
...
table(dat.taf.9$wave)
```
##Check how many participants answered each wave COPING: taf.felt_weeks_past.1
```{r}
