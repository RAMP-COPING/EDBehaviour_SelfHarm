...
This script identifies eating disorder cases within the COPING study (EDGI, GLAD, NBR participants) and RAMP.

To note: 
1) GLAD COPING includes self-report and algorithm-derived diagnoses, as GLAD participants were given the ED100K questions within the COPING baseline, as it was only an optional questionnaire in GLAD.
2) EDGI COPING includes only self-report diagnoses, as all participants will have already filled out the ED100K questions in EDGI as it was included in the signup, so therefore was not included in the EDGI COPING baseline
3) NBR COPING includes self-report and algorithm-derived diagnoses, as NBR participants were given the ED100K questions within the COPING baseline, as it was only an optional questionnaire in GLAD
4) RAMP includes only self-reported diagnoses, as RAMP participants were not given the ED100K questions at any point within RAMP, however were asked to self-report mental health diagnoses.

Therefore, this script includes eating disorder algorithms only for 1) GLAD COPING, 2) NBR COPING, but self-reported diagnoses for ALL cohorts, i.e., 1) GLAD COPING, 2) NBR COPING, 3) EDGI COPING, and 4) RAMP.

The first part of this script is the GLAD COPING and NBR COPING algorithms. 
...
### Read in ICB data: GLAD COPING ICB 
```{r}
...
matched_IDS_2 <- dat.an.glad.coping.id[dat.an.glad.coping.id$ID %in% dat.icb.glad.id$ID,]
...
#Work out lowest BMI as reported in COPING (NBR + GLAD)
BMI = kg/m2 

##Lowest weight in kg (GLAD COPING and NBR COPING)
```{r}
...
```
## Clean lowest weight in kg (GLAD + NBR)
```{r}
#Weight outlier
...
##Height at lowest weight in m (GLAD + NBR)
```{r}
...
## Work out lowest BMI (GLAD + NBR)
```{r}

# Calculated from height and weight **This should be based on min weight and height - have quite low values <10 - need to check
dat.ed$lowest_BMI_COPING <- NA

#Derived from height and weight
dat.ed$lowest_BMI_COPING <- 
  if_else(condition = is.na(dat.ed$lowest_BMI_COPING), 
         true = dat.ed$weight_kg_final/dat.ed$height_m_final^2, 
         false = NA_real_, #no self-reported BMI in COPING
         missing = NA_real_)

# Summary
summary(dat.ed$lowest_BMI_COPING)
...
```
#### DSM 5 AN (no subtype) - COPING GLAD and NBR
...
```{r}
dat.ed$ed.DSM5_COPING_AN_binary_numeric <- 
  with(dat.ed,
  dplyr::if_else(
#A. Restriction of energy intake relative to requirements,leading to a significantly low body weight in the context of age, sex, developmental trajectory, and physical health. Significantly low weight is defined as a weight that is less than minimally normal or, for children and adolescents, less than that minimally expected.
    (
    (
      lowest_BMI_COPING <= 18.55 #Lowest BMI as reported in COPING ED baseline questionnaire
      ) &
...
dat.ed$ed.DSM5_COPING_AN_binary <-
  recode_factor(dat.ed$ed.DSM5_COPING_AN_binary_numeric,
                "0" = "No DSM5 AN",
                "1" = "DSM5 AN"
                )

#Summary
freq(dat.ed$ed.DSM5_COPING_AN_binary)


#Check which cohort they are from
AN_cases <- dat.ed %>%
  filter(ed.DSM5_COPING_AN_binary == "DSM5 AN")

freq(AN_cases$cohort_name)
```

#### DSM 5 AN restricting - COPING GLAD and NBR
A. Restriction of energy intake relative to requirements,leading to a significantly low body weight in the context of age, sex, developmental trajectory, and physical health. Significantly low weight is defined as a weight that is less than minimally normal or, for children and adolescents, less than that minimally expected.
...
```{r}
dat.ed$ed.DSM5_COPING_AN_restricting_binary_numeric <- 
  with(dat.ed,
  dplyr::if_else(
#A. Restriction of energy intake relative to requirements,leading to a significantly low body weight in the context of age, sex, developmental trajectory, and physical health. Significantly low weight is defined as a weight that is less than minimally normal or, for children and adolescents, less than that minimally expected.
    (
...
#Recode numeric to factor
dat.ed$ed.DSM5_COPING_AN_restricting_binary <-
  recode_factor(dat.ed$ed.DSM5_COPING_AN_restricting_binary_numeric,
                "0" = "No DSM5 AN restricting",
                "1" = "DSM5 AN restricting"
                )

#Summary
freq(dat.ed$ed.DSM5_COPING_AN_restricting_binary)


#Check which cohort they are from
ANR_cases <- dat.ed %>%
  filter(ed.DSM5_COPING_AN_restricting_binary == "DSM5 AN restricting")
...
```{r}
dat.ed$ed.DSM5_COPING_AN_binge_purge_binary_numeric <- 
  with(dat.ed,
  dplyr::if_else(
#A. Restriction of energy intake relative to requirements,leading to a significantly low body weight in the context of age, sex, developmental trajectory, and physical health. Significantly low weight is defined as a weight that is less than minimally normal or, for children and adolescents, less than that minimally expected.
    (
...
dat.ed$ed.DSM5_ANBP_binary <-
  recode_factor(dat.ed$ed.DSM5_COPING_AN_binge_purge_binary_numeric,
...
dat.ed$ed.DSM5_COPING_BN_binary_numeric <- 
  with(dat.ed,
  dplyr::if_else(
...
dat.ed$ed.DSM5_COPING_BN_binary <-
  recode_factor(dat.ed$ed.DSM5_COPING_BN_binary_numeric,
                "0" = "No DSM-5 BN",
                "1" = "DSM-5 BN")

#Summary
freq(dat.ed$ed.DSM5_COPING_BN_binary)

#Check which cohort they are from
BN_cases <- dat.ed %>%
  filter(ed.DSM5_COPING_BN_binary == "DSM-5 BN")
...
dat.ed$ed.DSM5_COPING_BED_binary_numeric <- 
  with(dat.ed,
  dplyr::if_else(
#A. Recurrent episodes of binge eating. An episode of binge eating is characterized by both of the following:
...
#Recode numeric to factor
dat.ed$ed.DSM5_COPING_BED_binary <-
  recode_factor(dat.ed$ed.DSM5_COPING_BED_binary_numeric,
                "0" = "No DSM-5 BED",
                "1" = "DSM-5 BED")

#Summary
freq(dat.ed$ed.DSM5_COPING_BED_binary)
...
glad.coping.mhd.raw.id <- glad.coping.mhd.raw  %>%
    ...
    select(
      ID = externalDataReference, # ID
     cohort_name, # Sample
       mhd.COPING_anorexia_nervosa = mhd.anorexia_nervosa,
       mhd.COPING_atypical_anorexia_nervosa = mhd.atypical_anorexia_nervosa,
       mhd.COPING_bulimia_nervosa = mhd.bulimia_nervosa,
       mhd.COPING_bingeeating_disorder = mhd.bingeeating_disorder
    ) %>%
...
ramp.mhd.raw.id <- ramp.mhd.raw %>%
    ...
    select(
      ID = `Login ID`, # ID
      cohort_name, # Sample
      mhd.COPING_anorexia_nervosa = mhq.anorexia_nervosa,
      mhd.COPING_atypical_anorexia_nervosa = mhq.atypical_anorexia_nervosa,
      mhd.COPING_bulimia_nervosa = mhq.bulimia_nervosa,
     mhd.COPING_bingeeating_disorder = mhq.psychological_overeating_or_bingeeating_disorder
    ) %>%
    add_numeric(., exclude = exclude_cols) %>%
    mutate_if(is.numeric, ~na_if(., -88)) %>% # Recode missing values to NAs in numeric variables
    mutate_if(is.numeric, ~na_if(., -99)) %>%
...
dat.mhd <- rbind(nbr.mhd.raw.id,  #17453 people
                   glad.coping.mhd.raw.id, #12860 people
                 edgi.mhd.raw.id, #3,251 people
                 ramp.mhd.raw.id #10,900 people
)
...
## New column: self-reported AND/OR algorithm-derived ANR/ANBP (COPING) --> not working?
```{r}


dat.ed.mhd <- dat.ed.mhd %>%
  mutate(
    COPING_anorexia_group_numeric =
           case_when(mhd.COPING_anorexia_nervosa_numeric == 1 |
                        ed.DSM5_COPING_AN_restricting_binary_numeric == 1 |
                       ed.DSM5_COPING_AN_binge_purge_binary_numeric == 1 |
                       ed.DSM5_COPING_AN_binary_numeric == 1 ~ 1, #Self-reported or algorithm-derived 
                     
                     
                     mhd.COPING_anorexia_nervosa_numeric == 0 |
                       ed.DSM5_COPING_AN_restricting_binary_numeric == 0 |
                       ed.DSM5_COPING_AN_binge_purge_binary_numeric == 0 |
                       ed.DSM5_COPING_AN_binary_numeric == 0  ~ 0, #This will capture anyone with at least one 0 (rest will be NA)
                     
                     is.na(mhd.COPING_anorexia_nervosa_numeric) & 
                     is.na(ed.DSM5_COPING_AN_restricting_binary_numeric) &
                      is.na(ed.DSM5_COPING_AN_binge_purge_binary_numeric) &
                       is.na(ed.DSM5_COPING_AN_binary_numeric) ~ NA_real_  )) #This will capture anyone with ALL missing data
                   
                    
#Check freq                    
dat.ed.mhd %>%
  freq(COPING_anorexia_group_numeric)
           
#Add factor variable 
dat.ed.mhd <- dat.ed.mhd %>%
    mutate(
      COPING_anorexia_group =
        recode_factor(COPING_anorexia_group_numeric,
...
#Check freq #3553 with AN
  dat.ed.mhd %>%
    freq(COPING_anorexia_group,
         cumul = F)
...
dat.ed.mhd <- dat.ed.mhd %>%
  mutate(
    COPING_bulimia_group_numeric =
           case_when(mhd.COPING_bulimia_nervosa_numeric == 1 |
                       ed.DSM5_COPING_BN_binary_numeric == 1 ~ 1, #Self-reported or algorithm-derived 
                     
                     mhd.COPING_bulimia_nervosa_numeric == 0 &
                       ed.DSM5_COPING_BN_binary_numeric == 0 ~ 0, #Not self-reported and not algorithm-derived
                     
                     mhd.COPING_bulimia_nervosa_numeric == 0 &
                       is.na(ed.DSM5_COPING_BN_binary_numeric) ~ 0, #Not self-reported and missing for algorithm-derived 
                     
                     is.na(mhd.COPING_bulimia_nervosa_numeric) &
                       ed.DSM5_COPING_BN_binary_numeric == 0 ~ 0, #Missing self-reported and not algorithm-derived 
                     
                     is.na(mhd.COPING_bulimia_nervosa_numeric) &
                       is.na(ed.DSM5_COPING_BN_binary_numeric) ~ NA_real_ #Missing for both
           )
  )

#Check freq                    
dat.ed.mhd %>%
  freq( COPING_bulimia_group_numeric)
           
#Add factor variable 
dat.ed.mhd <- dat.ed.mhd %>%
    mutate(
       COPING_bulimia_group =
        recode_factor( COPING_bulimia_group_numeric,
...
#Check freq #2584 with BN
  dat.ed.mhd %>%
    freq(COPING_bulimia_group,
         cumul = F)
...
dat.ed.mhd <- dat.ed.mhd %>%
  mutate(
     COPING_bed_group_numeric =
           case_when(mhd.COPING_bingeeating_disorder_numeric == 1 |
                       ed.DSM5_COPING_BED_binary_numeric == 1 ~ 1, #Self-reported or algorithm-derived 
                     
                     mhd.COPING_bingeeating_disorder_numeric == 0 & #Not self-reported and not algorithm-derived
                       ed.DSM5_COPING_BED_binary_numeric == 0 ~ 0,
                     
                     mhd.COPING_bingeeating_disorder_numeric == 0 & #Not self-reported and missing for algorithm-derived 
                       is.na(ed.DSM5_COPING_BED_binary_numeric) ~ 0,
                     
                     is.na(mhd.COPING_bingeeating_disorder_numeric) & #Missing self-reported and not algorithm-derived
                       ed.DSM5_COPING_BED_binary_numeric == 0 ~ 0,
                     
                     is.na(mhd.COPING_bingeeating_disorder_numeric) & #Missing for both
                       is.na(ed.DSM5_COPING_BED_binary_numeric) ~ NA_real_
           )
  )

#Check freq                    
dat.ed.mhd %>%
  freq(COPING_bed_group_numeric)
           
#Add factor variable 
dat.ed.mhd <- dat.ed.mhd %>%
    mutate(
       COPING_bed_group =
        recode_factor(COPING_bed_group_numeric,
...
#Check freq #1902 with BED
  dat.ed.mhd %>%
    freq(COPING_bed_group,
         cumul = F)
...
```{r}
final_coping_data_ed <- dat.ed.mhd %>%
  select(ID,
         cohort_name,
         COPING_bed_group_numeric,
         COPING_bed_group,
         mhd.COPING_bingeeating_disorder,
         mhd.COPING_bingeeating_disorder_numeric,
         ed.DSM5_COPING_BED_binary,
         ed.DSM5_COPING_BED_binary_numeric,
         COPING_bulimia_group_numeric,
         COPING_bulimia_group,
         mhd.COPING_bulimia_nervosa_numeric,
         ed.DSM5_COPING_BN_binary_numeric,
         mhd.COPING_bulimia_nervosa,
         ed.DSM5_COPING_BN_binary,
         COPING_anorexia_group_numeric,
         COPING_anorexia_group,
         mhd.COPING_anorexia_nervosa_numeric,
         mhd.COPING_anorexia_nervosa,
         ed.DSM5_COPING_AN_restricting_binary_numeric,
         ed.DSM5_COPING_AN_binge_purge_binary_numeric,
         ed.DSM5_COPING_AN_binary_numeric) 
    
```

##Save dataset
```{r}
saveRDS(object = final_coping_data_ed,
        file = paste0("/Users/helenadavies/King's College London/MT-Translational Neuropsychiatric Genomics - Helena_Davies_PhD - Helena_Davies_PhD/EDBeh_SelfHarm/data_cleaned/COPING_RAMP_ED_data_cleaned", date, ".rds"))
