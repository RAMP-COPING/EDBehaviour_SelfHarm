...
ramp.dem.raw.id <- ramp.dem.raw %>% #new dataset with ID
  ...
  select(
         ...
         #Need to rename all to same as GLAD/EDGI/NBR
         dem2.living_situation_change_pandemic,
         dem2.living_alone.1,
         dem2.living_with_partner_or_spouse.1,             
         dem2.living_with_housemates.1,
         ...
         dem2.living_with_other_members_of_my_family.1,  
          dem2.spouse_partner_members_living.1 = dem2.spouse_members_partner_living.1,
         dem2.staying_live.2,                               
         dem2.staying_live.3,
         dem2.prefer_not_to_say.1,
         ...
         dem2.living_with_other_members_of_my_family,        
         dem2.spouse_partner_members_living = dem2.spouse_members_partner_living,
         dem2.staying_live,                                 
         dem2.staying_live.1, 
         dem2.prefer_not_to_say,
...
# Read in data - pan worry baseline EDGI
```{r}
if(EDGI == TRUE){
...
# Read in data - pan worry baseline GLAD
```{r}
if(GLAD == TRUE){
...
# Read in data - pan worry baseline NBR
```{r}
if(NBR == TRUE){
...
# Read in data - pan worry baseline RAMP
```{r}
if(RAMP == TRUE){
...
```
### Select variables
```{r GLAD losing somone due to COVID baseline select variables}
if(GLAD == TRUE){
exclude_cols_dem <- c("ID",
                  "cohort_name",
                  "endDate")
...
#Losing someone due to COVID - EDGI
```{r}
if(EDGI == TRUE) {
...
```
### Select variables
```{r EDGI losing somone due to COVIDselect variables}
if(EDGI == TRUE){
exclude_cols_dem <- c("ID",
                  "cohort_name",
                  "endDate")
...
#Losing someone due to COVID - NBR
```{r}
if(NBR == TRUE) {
...
### Select variables
```{r NBR losing somone due to COVID baseline select variables}
if(NBR == TRUE){
exclude_cols_dem <- c("ID",
                  "cohort_name",
                  "endDate")
...
#Losing someone due to COVID - RAMP
```{r}
if(RAMP == TRUE) {
...
```
### Select variables
```{r GLAD loneliness baseline select variables}
if(GLAD == TRUE){
exclude_cols_dem <- c("ID",
                  "cohort_name",
                  "endDate")
...
```
### Select variables
```{r RAMP loneliness baseline select variables}
if(RAMP == TRUE){
exclude_cols_dem <- c("ID",
                  "cohort_name",
                  "endDate")
...
```
### Select variables
```{r NBR loneliness baseline select variables}
if(NBR == TRUE){
exclude_cols_dem <- c("ID",
                  "cohort_name",
                  "endDate")
...
###Read in EDEQ baseline data (need this to assess current symptoms; although note that is only present in RAMP)
## RAMP baseline
### Read in data
...
##Follow up B virus info - COPING
```{r COPING follow up b virus cols}
if(COPING == TRUE){
...
### Select variables
```{r COPING follow up b virus select variables}
if(COPING == TRUE){
exclude_cols_dem <- c("ID",
                  "cohort_name",
                  "startDate_waves",
...
##Follow up B virus info - RAMP
```{r RAMP follow up b virus cols}
if(RAMP == TRUE){
...
### Select variables
```{r RAMP follow up b virus select variables}
if(RAMP == TRUE){
exclude_cols_dem <- c("ID",
                  "cohort_name",
                  "startDate_waves",
...
```
### Select variables
```{r RAMP dem follow up a select variables}
if(RAMP == TRUE){
exclude_cols_dem <- c("ID",
                  "cohort_name",
                  "startDate_waves",
...
```
### Select variables
```{r RAMP dem follow up b select variables}
if(RAMP == TRUE){
exclude_cols_dem <- c("ID",
                  "cohort_name",
                  "startDate_waves",
...
```
### Select variables
```{r COPING dem follow up a select variables}
if(COPING == TRUE){
exclude_cols_dem <- c("ID",
                  "cohort_name",
                  "startDate_waves",
...
coping.dem.followupa.raw.id.1 <- coping.dem.followupa.raw %>% #new dataset with ID
  ...
  select(
         ...
         dem.health_of_a_relative,
         dem.household_employment_status_key,
         dem.children__dependants_returning_to_school__other_education = dem.school_children_dependants_returning,
         dem.living_alone,
         dem.living_with_partner_or_spouse,
         dem.living_with_housemates,
         dem.age_children_living = dem.age_living_children,
...
```
### Select variables
```{r COPING dem follow up a ongoing select variables}
if(COPING == TRUE){
exclude_cols_dem <- c("ID",
                  "cohort_name",
                  "startDate_waves",
...
#Combine dem coping follow up A datasets 
```{r}
if(COPING == TRUE){
...
```
### Select variables
```{r COPING dem follow up b ongoing select variables}
if(COPING == TRUE){
exclude_cols_dem <- c("ID",
                  "cohort_name",
                  "startDate_waves",
...
### Select variables
```{r COPING loss follow up b select variables}
if(COPING == TRUE){
exclude_cols_dem <- c("ID",
                  "cohort_name",
                  "startDate_waves",
...
### Select variables
```{r RAMP loss follow up b select variables}
if(RAMP == TRUE){
exclude_cols_dem <- c("ID",
                  "cohort_name",
                  "startDate_waves",
...
1. AGE AT COVID ASSESSMENT (i.e., EDGI and GLAD COPING baseline questionnaire NOT GLAD/EDGI signup)
For RAMP and NBR, can just age at signup (this is effectively their COVID baseline)
```{r source script age}
if(NBR == TRUE | EDGI == TRUE | GLAD == TRUE | RAMP == TRUE) {
...
##RAMP - loss of loved one due to COVID 
...
##EDEQ SCREENER RAMP; FIRST - checking for number of entries per wave!

**Need to think about people who answered EDEQ screener in an 'A' timepoint = 11 people (think will just drop them for the plots, but for the regression should actually keep them)
...
##COPING - loss of loved one due to COVID - HEZ
...
##FIRST - EDEQ SCREENER -  checking for number of entries per wave!
...
saveRDS(object = dat.raw.taf.coping, file = "../data_cleaned/NEW_taf_coping.rds")

saveRDS(object = edeq.coping.followupb.screener.raw.id, file = "../data_cleaned/NEW_edeq_screener_coping.rds")

saveRDS(object = dat.raw.dem.coping.followup, file = "../data_cleaned/dem_followup_coping.rds")

saveRDS(object = coping.loss.followupb.raw.id, file = "../data_cleaned/loss_coping_followup.rds")


}

if(RAMP == TRUE) {
saveRDS(object = dat.raw.taf.ramp, file = "../data_cleaned/NEW_taf_ramp.rds")

saveRDS(object = edeq.ramp.followupb.screener.raw.id, file = "../data_cleaned/NEW_edeq_screener_ramp.rds")

saveRDS(object = dat.raw.dem.ramp.followup, file = "../data_cleaned/dem_followup_ramp.rds")

saveRDS(object = ramp.loss.followupb.raw.id, file = "../data_cleaned/loss_ramp_followup.rds")
...
##RAMP - VIRUS INFO - Want the LATER data entry from people who answered twice within a single wave 
[want ID AND wave to be unique]
```{r}
if(RAMP == TRUE) {
