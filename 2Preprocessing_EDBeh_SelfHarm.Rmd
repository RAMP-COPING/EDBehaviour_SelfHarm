---
title: "2Preprocessing_EDBeh_SelfHarm"
...
##CONTROLS
###Call script for recoding/grouping controls
```{r mental health diagnoses all cohorts}
if(NBR == TRUE | EDGI == TRUE | GLAD == TRUE | RAMP == TRUE) {
source("./Controls.R")
}
...
##Idenfitying duplicate IDs in a single timepoint
```{r}
if(RAMP == TRUE) {
...
##Remove duplicate IDs (as each wave is now a column, there should be NO dup IDs)
edeq.ramp.followupb.raw.id  <- edeq.ramp.followupb.raw.id  %>%
  filter(., !duplicated(ID)) 
