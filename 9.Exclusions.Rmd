...
# Read in data
```{r read in data}
dat_raw <- readRDS(paste0(filepath_saved_data, "final_dat_merged_2022-09-27.rds"))
...
dat_BE <- dat_BE_possible %>%
  filter(!is.na(NEW_binge_eating_pandemic))

nrow(dat_BE)-nrow(dat_BE_possible)
```
...
```
Of those who had no pre-pandemic self-harm (30,016), 68 were missing for new onset harmful ideation during the pandemic.
