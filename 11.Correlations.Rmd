...
BE_dat_corr_mat <- dat_dummy %>%
   filter(NEW_binge_eating_pandemic_NA == 0) %>%
  select(
...
LW_dat_corr_mat <- dat_dummy %>%
   filter(NEW_low_weight_pandemic_NA == 0) %>%
  select(
    "Being female" = "sex_binary_Female",
    "Minoritised gender" = "minoritised_gender_Yes",
   "16-25 years" = "age_category_COVID_baseline_16-25",
...
HI_dat_corr_mat <- dat_dummy %>%
   filter(NEW_harmful_ideation_pandemic_NA == 0) %>%
  select( 
 "Being female" = "sex_binary_Female",
   "Minoritised gender" = "minoritised_gender_Yes",
   "16-25 years" = "age_category_COVID_baseline_16-25",
...
SH_dat_corr_mat <- dat_dummy %>%
   filter(NEW_selfharm_pandemic_NA == 0) %>%
  select(
 "Being female" = "sex_binary_Female",
    "Minoritised gender" = "minoritised_gender_Yes",
   "16-25 years" = "age_category_COVID_baseline_16-25",
...
png(height = 900,
    width = 900,
    file = paste0(filepath_plots_corr, "new_LW_exposures_corr_matrix.png")
)
