...
## GLAD data
```{r GLAD read in data}
glad_dat <- readRDS(file = passte0(filepath_raw_latest_freeze, "coping_glad_opt/ocd_coping_glad_opt.rds"))
# Check
glad_dat %>%
  colnames()
glad_dat %>%
...
glad_dat.id <- glad_dat %>% #new dataset with ID
  ...
  remove_duplicates("recipientFirstName") %>% 
  add_column(sample = "GLAD", .after = "recipientFirstName") %>% # Create new sample column
  select(
         ID = recipientFirstName,# ID
         sample,
...
## EDGI data
```{r edgi read in data}
edgi_dat <- readRDS(file = paste0(filepath_raw_latest_freeze, "coping_edgi_opt/ocd_coping_edgi_opt.rds"))
# Check
edgi_dat %>%
  colnames()
edgi_dat %>%
...
edgi_dat.id <- edgi_dat %>% #new dataset with ID
  drop_na(recipientFirstName) %>% # Drop NAs
  remove_duplicates("recipientFirstName") %>% 
  add_column(sample = "EDGI", .after = "recipientFirstName") %>% # Create new sample column
  select(
         ID = recipientFirstName,# ID
         sample,
         ocd.strong_urges_urges_dirt = ocd.strong_urges_germs_thoughts,
...
## NBR data
```{r NBR read in data}
nbr_dat <- readRDS(file = paste0(filepath_raw_latest_freeze, "coping_nbr_opt/ocd_coping_nbr_opt.rds"))
...
nbr_dat.id <- nbr_dat %>% #new dataset with ID
  drop_na(recipientFirstName) %>% # Drop NAs
  remove_duplicates("recipientFirstName") %>% 
  add_column(sample = "NBR", .after = "recipientFirstName") %>% # Create new sample column
  select(
         ID = recipientFirstName,# ID
         sample,
         ocd.strong_urges_urges_dirt = ocd.strong_urges_germs_thoughts,
...
## RAMP data
```{r RAMP read in data}
ramp_dat <- readRDS(file = paste0(filepath_raw_latest_freeze, "ramp/ocd_ramp.rds"))
...
# Save dataset
```{r Save dataset}
saveRDS(object = dat_final, file = paste0(filepath_saved_data, "OCD_screener", date, ".rds")))
```
