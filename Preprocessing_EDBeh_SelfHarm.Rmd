...
taf.coping.followupa.raw.id.2 <- taf.coping.followupa.raw.2 %>% #new dataset with ID
  ...
  select(
         ID = externalDataReference, # ID
         cohort_name, # Sample
         startDate_wavesA_taf_ongoing = startDate,
         #endDate,
         taf.worth_living_life_people_followupA_ongoing = taf.worth_living_life_people,
         taf.felt_weeks_past_followupA_ongoing = taf.felt_weeks_past,
         taf.have_you_contemplated_harming_yourself_followupA_ongoing = taf.have_you_contemplated_harming_yourself_,
         taf.felt_weeks_past.1_followupA_ongoing = taf.felt_weeks_past.1,
         taf.meant_life_end_weeks_followupA_ongoing = taf.meant_life_end_weeks
         ) %>%
  add_numeric(., exclude = exclude_cols_dem) %>%
  mutate_if(is.numeric, ~na_if(., -88)) %>% # Recode missing values to NAs in numeric variables
  mutate_if(is.numeric, ~na_if(., -99)) %>%
...
```{r}
if(COPING == TRUE){
    taf.coping.followupa.raw.id <- merge(taf.coping.followupa.raw.id.2, taf.coping.followupa.raw.id.1, by = c("ID", "cohort_name"))
...
#List of follow up cohorts (COPING)
...
if(RAMP == TRUE | NBR ==TRUE | EDGI == TRUE| GLAD == TRUE |COPING == TRUE) {
...
##Should do this AFTER split by waves (i.e. one person should not have multiple data for one wave [e.g. duplicated ID in wave 1a dataset], but one person will have multiple wave data [e.g. wave 1a, wave 1b, etc])
