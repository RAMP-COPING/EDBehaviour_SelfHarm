...
##Restraint subscale - BASELINE
1 Restraint over eating
2 Avoidance of eating
3 Food avoidance
4 Dietary Rules
5 Empty stomach
```{r edeq ramp restraint subscale baseline scoring, eval=FALSE, include=FALSE}
if(RAMP == TRUE){
  #Check that scores for all items are between 0 - 6
freq(edeq.ramp.baseline.raw.id$edeq.deliberately_limit_amount_influence_baseline_numeric)

  freq(edeq.ramp.baseline.raw.id$edeq.av)
...
taf.coping.followupa.raw.id.2 <- taf.coping.followupa.raw.2 %>% #new dataset with ID
  ...
  select(
         ID = externalDataReference, # ID
         cohort_name, # Sample
         startDate_wavesA_ongoing = startDate,
         #endDate,
...
freq(dat.raw.no.dup$wave_A_taf)

  freq(dat.raw.no.dup$wave_B_taf)
}
```
