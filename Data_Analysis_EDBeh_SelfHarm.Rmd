...
```{r}
#get rid of nas
dat.no.nas.edeq <- dat.edeq.long[!is.na(dat.edeq.long$edeq.actively_been_monitoring_your_shape_or_weight),]

#change wave to factor
#dat.no.nas$wave_factor <- as.factor(dat.no.nas$wave)

#change taf to factor
dat.no.nas.edeq$edeq.actively_been_monitoring_your_shape_or_weight <- factor(dat.no.nas.edeq$edeq.actively_been_monitoring_your_shape_or_weight)
```
...
dat.no.nas.edeq <- dat.no.nas.edeq %>%
...
summary(dat.no.nas.edeq$wave_factor)
...
dat_plot <- dat.no.nas %>%
  group_by(wave_factor) %>%
  dplyr::summarise(wcount = (table(dat_pltaf.worth_living_life_people) / sum(table(taf.worth_living_life_people)))[c(2,1,3,4)])   %>%
  ungroup() %>%
  mutate(taf.worth_living_life_people = rep(levels(dat.no.nas[["taf.worth_living_life_people"]]),
                    length(levels(dat.no.nas[["wave_factor"]]))))
