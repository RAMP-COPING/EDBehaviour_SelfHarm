...
x <- reshape2::melt(dat.taf_practice, id.vars = c("ID",
                                "cohort_name",
                                "sex",
                                "gender_unc",
                                "uk_location",
                                ...
                                "key_worker_numeric",
                                "taf.pandemic_felt_baseline"),
          measure.vars = c("taf.felt_weeks_past_baseline"),
          value.name = c("felt.weeks.baseline"))
   colnames(x)    
   
   ##rename 'variable' column name  to 'wave', and variables within it to "baseline"
   
x$felt.weeks.baseline <- paste(y$variable,y$variable)

x$wave <- paste0("baseline",22:nrow(x))
          
y <- reshape2::melt(x, id.vars = c("ID",
                                "cohort_name",
                                "sex",
                                "gender_unc",
                                "uk_location",
                                ...
                                "felt.weeks.baseline",
                                "variable"),
          measure.vars = c("taf.pandemic_felt_baseline"),
          value.name = c("pandemic_felt_baseline"))
...
dat.taf <- dat.taf %>%
  pivot_longer(
    ...
    names_to = "wave_taf",
    values_to = "value") %>%
  separate(wave_taf, into = c("taf", "wave"), sep = "_W", remove = TRUE) %>%
  pivot_wider(
     names_from = "taf",
     values_from = "value")
...
ggplot(dat.taf,
       aes(taf.worth_living_life_people_numeri, 
           fill = wave)) +
  geom_bar(size = 1,
           shape = 23,
           colour = "black",
