...
dat <- dat %>%
  mutate(outcome_phase_passive_suicidal_ideation =
           case_when(is.na(taf.felt_weeks_past_.Wave_1a) &
                      is.na(taf.worth_living_life_people_.Wave_1a) &
                      is.na(taf.worth_living_life_people_.Wave_1a) &
                      is.na(taf.felt_weeks_past_.Wave_1b) &
                      is.na(taf.worth_living_life_people_.Wave_1b) &
                      ...
                      is.na(taf.worth_living_life_people_.Wave_12b) ~ "Not answered",
...
dat <- dat %>%
  mutate(exposure_phase_positive_covid_test =
           case_when(  is.na(virus.back_positive_antibodies_covid_.Wave_1b )   &
                       ...
                       is.na(virus.back_positive_antibodies_covid_.Wave_12b)  ~ "Not answered", # Then, everyone who is NA will have answered "No" at least once, even if they are missing for all other phases. The only people we can not assume an answer for (i.e., has experienced or has not experienced) is those who never answered the question throughout the whole time period
...
dat <- dat %>%
  mutate(positive_covid_test_BE_1b =
           case_when(exposure_phase_positive_covid_test == "1b" & # First positive COVID test at phase 1b
                      is.na(outcome_phase_binge_eating) ~ "No outcome", # Never experienced outcome (people who are all NA are coded as "Not answered")
...
dat <- dat %>%
  mutate(positive_covid_test_before_BE =
           case_when(# First positive test NOT before first experience of binge eating
                    ...
                    positive_covid_test_BE_12b == "To drop" ~ "Positive test after BE",
                    
                    # Not answered whether they'd taken a test
                    exposure_phase_positive_covid_test == "Not answered" ~ "Missing whether taken test", # Need to be dropped
                   ...
                   # Experienced outcome but not exposure
                    is.na(exposure_phase_positive_covid_test) &
                     !is.na(outcome_phase_binge_eating) ~ "Outcome but no exposure",
                   
                       # Experienced neither outcome nor exposure
                    is.na(exposure_phase_positive_covid_test) &
                     is.na(outcome_phase_binge_eating) ~ "Neither exposure nor outcome",
                    ...
                    # Not had a test
                     ...
                     virus.past_surveys_include_reported_.Wave_12b == "No"  ~ "Not had a test", # Will be grouped as 'Not positive' so order doesn't matter
                    ...
                    # Unsure/inconclusive test result 
                     ...
                     virus.back_positive_antibodies_covid_.Wave_12b == "Unsure/inconclusive"  ~ "Inconclusive test", # Will be grouped as 'Not positive' so order doesn't matter
                     ...
                     virus.back_positive_antibodies_covid_.Wave_12b == "No"  ~ "Negative test" # Will be grouped as 'Not positive' so order doesn't matter
      )
)
...
dat <- dat %>%
  mutate(positive_covid_test_before_BE_final =
           case_when(positive_covid_test_before_BE == "Positive test after BE" | # Need to drop people who first experienced outcome before first experience of exposure
                     ...
                     positive_covid_test_before_BE == "Negative test" |
...
dat <- dat %>%
  mutate(positive_covid_test_LW_1b =
           case_when(exposure_phase_positive_covid_test == "1b" & # First positive COVID test at phase 1b
                     is.na(outcome_phase_low_weight) ~ "No outcome", # Never experienced outcome (people who are all NA are coded as "Not answered")
...
dat <- dat %>%
  mutate(positive_covid_test_LW_12b =
           case_when(exposure_phase_positive_covid_test == "12b" &
                     ...
                     exposure_phase_positive_covid_test == "12b" & # First positive COVID test at phase 12b
                     is.na(outcome_phase_low_weight) ~ "No outcome", # Not experienced low weight during COPING
...
dat <- dat %>%
  mutate(positive_covid_test_before_LW =
           case_when(# First positive COVID test NOT before first experience of low weight
                    ...
                    positive_covid_test_LW_12b == "To drop" ~ "Positive test after low weight", # Need to be dropped
                    ...
                    # First positive COVID test before first experience of low weight
                    ...
                    # Experienced outcome but not exposure
                    is.na(exposure_phase_positive_covid_test) &
                     !is.na(outcome_phase_low_weight) ~ "Outcome but no exposure",
                   
                       # Experienced neither outcome nor exposure
                    is.na(exposure_phase_positive_covid_test) &
                     is.na(outcome_phase_low_weight) ~ "Neither exposure nor outcome",
...
dat <- dat %>%
  mutate(positive_covid_test_before_LW_final =
           case_when(positive_covid_test_before_LW == "Positive test after low weight" | # Need to drop people who first experienced outcome before first experience of exposure
                     ...
                     positive_covid_test_before_LW == "Negative test" |
...
dat <- dat %>%
  mutate(positive_covid_test_PSS_1b =
           case_when(exposure_phase_positive_covid_test == "1b" & # First positive COVID test at phase 1b
                     is.na(outcome_phase_passive_suicidal_ideation) ~ "No outcome", # Never experienced outcome (people who are all NA are coded as "Not answered")
...
+++ HAL HERE - check below chunk works/makes sense, then do other self-harm thoughts and behaviours
NB: Have combined "Not had a test", "Negative test", and "Inconclusive test" as "Not positive", i.e., the only thing we count as positive is if someone has a taken a test and had a positive result. Everything else is "Not positive test" other than those who had not answered whether they had taken a test.
+++ See comments above
...
dat <- dat %>%
  mutate(positive_covid_test_before_PSS =
           case_when(# First positive COVID test NOT before first experience of low weight
                    ...
                    positive_covid_test_PSS_12b == "To drop" ~ "Positive test after passive suicidal ideation", # Need to be dropped
                    
                     # Not answered whether they'd taken a test
                    exposure_phase_positive_covid_test == "Not answered" ~ "Missing whether taken test", # Need to be dropped
                    
                     # Not answered whether they'd experienced binge eating 
                    outcome_phase_passive_suicidal_ideation == "Not answered" ~ "Missing whether experienced low weight", # Need to be dropped
                    
                    # First positive COVID test before first experience of passive suicidal ideation
                    ...
                    positive_covid_test_PSS_12b == "No outcome" ~ "Exposure but no outcome",
                     ...
                     # Experienced outcome but not exposure
                    is.na(exposure_phase_positive_covid_test) &
                     !is.na(outcome_phase_passive_suicidal_ideation) ~ "Outcome but no exposure",
                   
                       # Experienced neither outcome nor exposure
                    is.na(exposure_phase_positive_covid_test) &
                     is.na(outcome_phase_passive_suicidal_ideation) ~ "Neither exposure nor outcome",
