...
This script derives variables reading for regression analyses.
...
## NB: Need to think about whether we want all the exposures to have occured BEFORE the outcomes (as Moritz suggested)
...
I have separated out those who are a key worker (and thus employed), those who are not a key worker but are employed, those who are unemployed (and thus not a key worker), and those who are students,
...
```{r Positve COVID test result exposure}
dat <- dat %>%
  mutate(covid_test =
           case_when(
...
dat <- dat %>%
  mutate(positive_covid_test_final =
           case_when(covid_test == "Negative test" |
                     covid_test == "Not had a test" |
                     covid_test == "Inconclusive test"  ~ "No",
                     
                     covid_test == "Positive test" ~ "Yes")
  )

# Check
dat %>%
  freq(positive_covid_test_final) 
```
